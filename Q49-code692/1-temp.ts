...
function highFive(items: number[][]): number[][] {
  ...
  return (
    [...record.entries()]
      .map(([id, scores]) => {
        const heap = maxHeap<number>((a, b) => a > b);
        scores.forEach((val) => heap.add(val));
        const avg = Array.from({ length: 5 }, heap.remove).reduce(
          (acc, val) => acc + val,
          0
        );
        return [id, ~~(avg / 5)];
...
function maxHeap<T>(compare: (a: T, b: T) => boolean) {
  const heap: Array<T> = [];
  return {
    size: () => heap.length,
    empty: () => heap.length === 0,
